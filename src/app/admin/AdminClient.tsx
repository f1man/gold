...
type SettingVal = { type: string; value: number };
type SettingsState = {
  pure_buy: SettingVal;
...
export default function AdminClient({ baseValue }: AdminClientProps) {
  ...
  const calculatePreview = (original: number, setting: SettingVal) => {
    ...
    if (type === '+원') result = original + value;
    else if (type === '-원') result = original - value;
    else if (type === '+%') result = original + (original * (value / 100));
    else if (type === '-%') result = original - (original * (value / 100));
      ...
      <div className={styles.section} key={category}>
        ...
        <div className={styles.flexRow} style={{ marginBottom: '1rem' }}>
          ...
          <input 
            ...
            value={setting.value} 
            onChange={(e) => handleSettingChange(category, 'value', Number(e.target.value) || 0)}
